...
13. **PackTierRegistry** (spoke) — UUPS-upgradeable singleton; inherits `PermissionConsumer`; stores per-`(machine, tokenId, packId)` tier assignments for all PackMachine clones (mirrors the `PackRegistry` pattern but keyed by token, not pack). Write functions (`setTier`, `deleteTier`, `deleteAllTiers`) gated by `onlyRegisteredMachine` modifier — requires `IPackMachineFactory(factory).isPackMachine(msg.sender)` — so only real clones can write their own tier data. `getTier(machine, tokenId, packId)` is public-read. Wired bidirectionally: `factory.setPackTierRegistry(address)` (gated `DEFAULT_ADMIN_ROLE`) + `packTierRegistry.setFactory(address)` (gated `DEFAULT_ADMIN_ROLE`); deployed and wired by `deploy-pack-machine.ts`. Clones access it via `IPackMachineFactory(factory).packTierRegistry()`. ERC-7201 storage slot: `erc7201:nettyworth.storage.PackTierRegistry`.
...
- **Per-pack card tiers (6 tiers)**: each card (tokenId) can hold a different tier in each pack — e.g. card A is Rare in pack 0 but Common in pack 1. Tiers: Base(0) / Common(1) / Uncommon(2) / Rare(3) / Ultra Rare(4) / Grail(5). Per-(pack,tier) data lives in `PackTierRegistry` keyed by `(machine, tokenId, packId)`. Clones store `packTierPools[packId][tier]` — the prize pool arrays partitioned by pack and tier.
- **Flat-encoding deposit API**: `deposit(tokenIds, packCounts, packIds, tiers, owner)` — `packCounts[i]` consecutive entries in the flat `packIds`/`tiers` arrays give token `i`'s (pack,tier) assignments. A token can be deposited into multiple packs in a single call (each with a different tier). Duplicate pack for a token reverts `PackMachine__InvalidPackRef`.
- **Per-(pack,tier) FMV bounds required at deposit**: `PackRegistry.setPackTierFmvBounds(machine, packId, minFmv[6], maxFmv[6])` must be set for every (packId, tier) pair before depositing tokens — `deposit` reverts `PackMachine__TierFmvUnset(packId, tier)` if `maxFmv[tier] == 0`, or `PackMachine__FmvOutOfRange` if the token's appraisal is outside bounds. Values in payment-token base units; gated `PACK_OPERATOR_ROLE`.
- **Dormant tier map restores per-pack tiers on re-deposit**: the `packTokenTier[tokenId][packId]` map on each clone survives a win (not deleted). `depositFromPool` (called by BuybackPool / AssetLendingPool) reads this dormant map to restore the correct per-pack tier automatically — no additional tier parameter needed from the caller.

## Project Layout

- `contracts/` — Solidity source files
  - `contracts/interfaces/` — Interface definitions (`IPermissionManager`, `ITransferValidator`, `IPackMachine`, `IPackMachineFactory`, `IPackVRFRouter`, `IBuybackPool`, `ISignatureTransfer`, `IAssetLendingPool`, `IAssetNFT`, `IFeeController`, `INettyWorthMarketplace`, `IP2PTradeEscrow`, `IPackRegistry`, `IPackTierRegistry`)
...
- `nettyworth-next/` — frontend repo (Next.js app; separate repo checked out alongside the contracts)
- `nettyworth-api/` — backend repo (API; separate repo checked out alongside the contracts)
