...
function resolveFromEnvOrDeployments(
    ...
    console.error(`${label} not found. Set ${envKey} env var or deploy ${deploymentKey} first.`);
    process.exit(1);
}
...
const permissionManagerProxy = resolveFromEnvOrDeployments(
    "PERMISSION_MANAGER_PROXY", "PermissionManager", data, "PermissionManager proxy"
);
const feeControllerProxy = resolveFromEnvOrDeployments(
    "FEE_CONTROLLER_PROXY", "FeeController", data, "FeeController proxy"
);
const lendingPoolProxy = resolveFromEnvOrDeployments(
    "LENDING_POOL_PROXY", "AssetLendingPool", data, "AssetLendingPool proxy"
);
const assetNFTProxy = resolveFromEnvOrDeployments(
    "ASSET_NFT_PROXY", "AssetNFT", data, "AssetNFT proxy"
);
...
if (process.env.PAYMENT_TOKEN) {
    ...
    console.error("PAYMENT_TOKEN env var is required (ERC20 address, e.g. USDC).");
    process.exit(1);
}
...
if (connection.networkConfig.type === "http") {
    const ok = await confirm(
        ...
        permissionManagerProxy, feeControllerProxy, lendingPoolProxy,
        assetNFTProxy, paymentToken, treasury,
    );
...
console.log("[4/6] Authorizing marketplace on AssetLendingPool (setMarketplace)...");
const pool = await viem.getContractAt("AssetLendingPool", lendingPoolProxy);
await pool.write.setMarketplace([proxy.address]);
...
if (process.env.SKIP_NFT_WIRING !== "true") {
    await nft.write.setPaymentToken([paymentToken]);
    await nft.write.setTreasury([treasury]);
    await nft.write.setFeeController([feeControllerProxy]);
    await nft.write.setLendingPool([lendingPoolProxy]);
    console.log("  AssetNFT: paymentToken, treasury, feeController, lendingPool set");
} else {
    console.log("  SKIP_NFT_WIRING=true — skipping AssetNFT wiring (call setters manually)");
}
...
const adminHasMarketplaceRole = await pm.read.hasProtocolRole([MARKETPLACE_ROLE, deployerAddress]);

const errors: string[] = [];
if (actualMarketplace.toLowerCase() !== proxy.address.toLowerCase()) {
    errors.push(`  pool.marketplace: expected "${proxy.address}", got "${actualMarketplace}"`);
}
...
console.log(`Admin MARKETPLACE_ROLE: ${adminHasMarketplaceRole ? "granted ✓" : "NOT granted (grant manually for keeper)"}`);
console.log("====================================================\n");
