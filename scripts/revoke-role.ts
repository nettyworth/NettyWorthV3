...
// ─── Validate ACCOUNT env var (comma-separated list supported) ────────────────
const rawAccount = process.env.ACCOUNT;
if (!rawAccount) {
  console.error("Missing required env var: ACCOUNT");
  console.error(
    "Usage: ROLE=<name> ACCOUNT=0x<address>[,0x<address>...] hardhat run scripts/revoke-role.ts --network <network>",
  );
  process.exit(1);
}
const targetAccounts: `0x${string}`[] = [];
for (const raw of rawAccount
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean)) {
  try {
    targetAccounts.push(getAddress(raw) as `0x${string}`);
  } catch {
    console.error(`Invalid ACCOUNT address: "${raw}"`);
    process.exit(1);
  }
}
if (targetAccounts.length === 0) {
  console.error("No valid addresses provided in ACCOUNT.");
  process.exit(1);
}
...
// ─── Idempotency check — keep only accounts that currently hold the role ──────
const accountsToRevoke: `0x${string}`[] = [];
for (const account of targetAccounts) {
  const held = await pm.read.hasProtocolRole([roleHash, account]);
  if (held) {
    accountsToRevoke.push(account);
  } else {
    console.log(
      `${account} does not hold ${roleName} on PermissionManager ${proxyAddress}. Skipping.`,
    );
  }
}
if (accountsToRevoke.length === 0) {
  console.log(`\nNothing to do — no target account holds ${roleName}.`);
  process.exit(0);
}
...
for (let i = 0; i < accountsToRevoke.length; i++) {
  const account = accountsToRevoke[i]!;
  console.log(
    `\n[${i + 1}/${accountsToRevoke.length}] Calling revokeRole(${roleName}, ${account})...`,
  );
  const txHash = await pm.write.revokeRole([roleHash, account], {
  ...
  // ─── Verify ─────────────────────────────────────────────────────────────────
  const stillHeld = await pm.read.hasProtocolRole([roleHash, account]);
  if (stillHeld) {
    console.error(
      `CRITICAL: hasProtocolRole returned true after revokeRole! tx: ${txHash}`,
    );
    process.exit(1);
  }
  console.log(`  ${account} no longer holds ${roleName} ✓`);
...
for (const r of revocations) {
  console.log(`Account:    ${r.account}  tx: ${r.txHash}`);
}
console.log("===========================\n");

// ─── Persist audit trail (live networks only) ─────────────────────────────────
if (connection.networkConfig.type === "http") {
  const existing = (deploymentData["RoleRevocations"] as unknown[]) ?? [];
  deploymentData["RoleRevocations"] = [...existing, ...revocations];
