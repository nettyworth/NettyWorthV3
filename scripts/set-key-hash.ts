...
// ─── ERC-7201 storage slot for PackVRFRouterStorage (base + 2) ───────────────
...
//   +2  keyHash (bytes32) ← this slot
//   +3  callbackGasLimit (uint32, 4 bytes, lowest) | requestConfirmations (uint16, next 2 bytes)
const KEY_HASH_STORAGE_SLOT =
    "0x1a86f08caea8a771089b03dcbcd6d44d4b9a0be22a0569919ff7e908d7550702" as const;

// ─── Parse VRF_KEY_HASH ───────────────────────────────────────────────────────
const rawKeyHash = process.env.VRF_KEY_HASH;

if (!rawKeyHash || !/^0x[0-9a-fA-F]{64}$/.test(rawKeyHash)) {
    console.error(
        `Invalid or missing VRF_KEY_HASH: "${rawKeyHash ?? ""}". Must be a 0x-prefixed 32-byte hex string (66 characters total).`,
    );
    process.exit(1);
}
const newKeyHash = rawKeyHash.toLowerCase() as `0x${string}`;
...
async function confirmUpdate(
    ...
    console.log(`Current:     ${oldKeyHash}`);
    console.log(`New:         ${newKH}`);
    console.log("==========================\n");
...
if (process.env.PACK_VRF_ROUTER_PROXY) {
    proxyAddress = getAddress(
        process.env.PACK_VRF_ROUTER_PROXY,
    ) as `0x${string}`;
    try {
    ...
    if (!entry?.proxy) {
        console.error(
            "PackVRFRouter proxy address not found in deployment file.",
        );
...
// ─── Read current keyHash from ERC-7201 storage ───────────────────────────────
const preSlotValue = await publicClient.getStorageAt({
    address: proxyAddress,
    slot: KEY_HASH_STORAGE_SLOT,
});
const currentKeyHash = (preSlotValue ?? "0x" + "0".repeat(64)).toLowerCase();

console.log(`\nCurrent keyHash: ${currentKeyHash}`);

if (currentKeyHash === newKeyHash) {
    console.log(`keyHash is already ${newKeyHash}. Nothing to do.`);
    process.exit(0);
}
...
// ─── Send transaction ─────────────────────────────────────────────────────────
console.log(`\n[1/2] Calling setKeyHash(${newKeyHash})...`);
const txHash = await router.write.setKeyHash([newKeyHash as `0x${string}`], {
...
const verifiedKeyHash = (postSlotValue ?? "").toLowerCase();

if (verifiedKeyHash !== newKeyHash) {
    console.error(
        `CRITICAL: Storage mismatch after update! Expected ${newKeyHash}, got ${verifiedKeyHash}`,
    );
    process.exit(1);
}
console.log(`  keyHash confirmed: ${verifiedKeyHash} ✓`);

console.log("\n=== setKeyHash Complete ===");
console.log(`Network: ${connection.networkName} (chainId: ${chainId})`);
console.log(`Proxy:   ${proxyAddress}`);
console.log(`Old:     ${currentKeyHash}`);
console.log(`New:     ${verifiedKeyHash}`);
...
if (connection.networkConfig.type === "http") {
    ...
    deploymentData["PackVRFRouter"] = {
        ...routerEntry,
        keyHash: newKeyHash,
        keyHashUpdatedAt: new Date().toISOString(),
    };
